...
function getActionVariant(acao = "") {
  ...
  if (
    /suspensão|exclusão|remoção|falhado|rejeição|cancelamento|inativação/.test(
      lower,
    )
  )
    return "error";
  if (/penalidade|status|bloqueio|atualização|alteração/.test(lower))
    return "warning";
  if (
    /cadastro|criação|restauração|ativação|aceitação|nova solicitação/.test(
      lower,
    )
  )
    return "success";
...
export function Auditoria() {
  ...
  async function handleExportPdf() {
    ...
    try {
      ...
      const tabLabel =
        activeTab === "app" ? "App — Usuários" : "Painel Admin/Dev";
      ...
      doc.setFontSize(9);
      doc.text(`Gerado em ${hojeFormatado}`, pageW - 12, 13, {
        align: "right",
      });
      ...
      autoTable(doc, {
        ...
        body: tabLogs.map((log) =>
          isApp
            ? [
                formatDate(log.criado_em),
                log.usu_nome ||
                  log.admin_nome ||
                  (log.usu_id ? `Usuário #${log.usu_id}` : "—"),
        ...
        didDrawPage: (hookData) => {
          ...
          doc.setFont("helvetica", "normal");
          doc.text(
            `Total: ${totalRows} registro${totalRows !== 1 ? "s" : ""}`,
            12,
            pageH - 6,
          );
          doc.text(
      ...
      <div className={styles.toolbar}>
        <form className={styles.filters} onSubmit={handleFilterSubmit}>
          ...
          {(appliedFilters.acao ||
            ...
            <button
              className={styles.filterBtnGhost}
              type="button"
              ...
              <tbody>
                {visibleLogs.map((log, index) => (
                  ...
                  >
                    ...
                    <td>
                      <span
                        className={`badge badge-${getActionVariant(log.acao)}`}
                      >
                        {translateAcao(log.acao)}
                      </span>
                    </td>
