...
-- [C] TESTE: Solicitações pendentes do passageiro com motorista e rota (GET /solicitacoes/usuario/:id)
SELECT
    s.sol_id, s.car_id, s.sol_vaga_soli, s.sol_status,
...
LEFT JOIN PONTO_ENCONTROS origem ON origem.pon_id = (
    ...
    ORDER BY pe.pon_ordem IS NULL, pe.pon_ordem ASC, pe.pon_id ASC LIMIT 1
)
...
WHERE s.usu_id_passageiro = 2  -- substituir pelo ID real
  AND s.sol_status = 1
